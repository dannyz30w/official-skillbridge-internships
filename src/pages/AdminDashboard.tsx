...
const AdminDashboard = () => {
  ...
  const loadBizNames = async (listings: Listing[]) => {
    const ids: string[] = [...new Set(listings.map(l => l.business_id))] as string[];
    ...
    <div className="min-h-screen flex bg-background">
        ...
        <nav className="flex-1 mt-4 space-y-1">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={tab === t.id ? { background: 'rgba(79, 70, 229, 0.1)', borderLeft: '3px solid hsl(243, 75%, 59%)' } : { background: 'transparent' }}>
              <t.icon className="h-4 w-4" />{t.label}
              {t.id === 'queue' && pendingCount > 0 && <span className="ml-auto text-xs font-semibold text-destructive-foreground rounded-full px-2 py-0.5" style={{ background: '#FF3B30' }}>{pendingCount}</span>}
            </button>
          ))}
        </nav>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 liquid-glass z-50 px-4 py-3" style={{ height: 56 }}>
        ...
        <div className="flex gap-1 mt-3 overflow-x-auto pb-1">{TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-fast ${tab === t.id ? 'text-primary' : 'text-muted-foreground'}`}
            style={tab === t.id ? { background: 'rgba(79, 70, 229, 0.1)' } : {}}>
            {t.label}{t.id === 'queue' && pendingCount > 0 && <span className="ml-1 text-xs font-semibold text-white rounded-full px-1.5" style={{ background: '#FF3B30' }}>{pendingCount}</span>}
          </button>
        ))}</div>
      </div>
      ...
      <main className="flex-1 md:ml-60 p-4 md:p-8 pt-28 md:pt-8">
        <motion.div key={tab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease }}>
          {tab === 'queue' && <div className="stagger-children">
            ...
            {loading ? <div className="skeleton-shimmer h-32 w-full" /> : pending.length === 0 ? (
              ...
              <div key={l.id} className="glass-card rounded-2xl p-5 card-hover">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div><p className="text-[13px] font-medium text-muted-foreground uppercase tracking-wider">{bizNames[l.business_id] || '...'}</p><h3 className="font-bold text-lg mt-1">{l.title}</h3>
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => setViewListing(l)} className="h-9 px-3 rounded-xl text-sm text-muted-foreground hover:text-foreground transition-fast btn-press" style={{ background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(0,0,0,0.08)' }}><Eye className="h-4 w-4" /></button>
                    <button onClick={() => approve(l.id)} className="h-9 px-4 rounded-xl text-sm font-semibold text-white btn-press" style={{ background: '#34C759' }}>Approve</button>
                    <button onClick={() => reject(l.id)} className="h-9 px-4 rounded-xl text-sm font-semibold text-white btn-press" style={{ background: '#FF3B30' }}>Reject</button>
                  </div>
                </div>
              </div>
            ))}</div>}
          ...
          {tab === 'listings' && <div>
            ...
            {loading ? <div className="skeleton-shimmer h-32 w-full" /> : <div className="space-y-3">{allListings.map(l => (
              <div key={l.id} className="glass-card rounded-2xl p-4 flex items-center justify-between card-hover">
                <div><p className="text-[13px] text-muted-foreground">{bizNames[l.business_id] || '...'}</p><h3 className="font-semibold">{l.title}</h3></div>
          ...
          {tab === 'interns' && <div>
            ...
            {loading ? <div className="skeleton-shimmer h-32 w-full" /> : <div className="grid gap-4 sm:grid-cols-2">{interns.map(i => (
              <div key={i.id} className="glass-card rounded-2xl p-5 card-hover">
                ...
                {i.skills?.length > 0 && <div className="flex flex-wrap gap-1 mt-2">{i.skills.map((s, idx) => <span key={idx} className="badge-remote text-[11px]">{s}</span>)}</div>}
                <button onClick={() => setConfirmDelete({ type: 'intern', userId: i.user_id })} className="mt-3 text-[13px] font-medium flex items-center gap-1 transition-fast btn-press" style={{ color: '#FF3B30' }}><Trash2 className="h-3 w-3" />Delete Profile</button>
              </div>
            ))}</div>}
          </div>}
          ...
          {tab === 'businesses' && <div>
            ...
            {loading ? <div className="skeleton-shimmer h-32 w-full" /> : <div className="grid gap-4 sm:grid-cols-2">{businesses.map(b => (
              <div key={b.id} className="glass-card rounded-2xl p-5 card-hover">
                ...
                <button onClick={() => setConfirmDelete({ type: 'business', userId: b.user_id })} className="mt-3 text-[13px] font-medium flex items-center gap-1 transition-fast btn-press" style={{ color: '#FF3B30' }}><Trash2 className="h-3 w-3" />Delete Account</button>
              </div>
            ))}</div>}
          </div>}
          ...
          {tab === 'settings' && <div>
            ...
            <button onClick={signOut} className="h-[44px] px-6 rounded-xl text-[15px] font-semibold text-white btn-press" style={{ background: '#FF3B30', transition: 'all 0.2s cubic-bezier(0.34, 1.56, 0.64, 1)' }}>Sign Out</button></div>
          </div>}
        </motion.div>

        {/* View listing modal */}
        {viewListing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.2)', backdropFilter: 'blur(8px)' }} onClick={() => setViewListing(null)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, ease }}
              className="glass-card rounded-2xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <div className="mt-6 flex gap-2 justify-end">
                <button onClick={() => { approve(viewListing.id); setViewListing(null); }} className="h-9 px-4 rounded-xl text-sm font-semibold text-white btn-press" style={{ background: '#34C759' }}>Approve</button>
                <button onClick={() => { reject(viewListing.id); setViewListing(null); }} className="h-9 px-4 rounded-xl text-sm font-semibold text-white btn-press" style={{ background: '#FF3B30' }}>Reject</button>
                <button onClick={() => setViewListing(null)} className="h-9 px-4 rounded-xl text-sm font-medium btn-press" style={{ background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(0,0,0,0.08)' }}>Close</button>
              </div>
            </motion.div>
          </div>
        )}
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.2)', backdropFilter: 'blur(8px)' }} onClick={() => setConfirmDelete(null)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3, ease }}
              ...
              <div className="mt-6 flex gap-2 justify-end">
                <button onClick={() => setConfirmDelete(null)} className="h-9 px-4 rounded-xl text-sm font-medium btn-press" style={{ background: 'rgba(255,255,255,0.6)', border: '1px solid rgba(0,0,0,0.08)' }}>Cancel</button>
                <button onClick={handleDelete} className="h-9 px-4 rounded-xl text-sm font-semibold text-white btn-press" style={{ background: '#FF3B30' }}>Delete</button>
              </div>
            </motion.div>
          </div>
        )}
