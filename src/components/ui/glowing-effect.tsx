...
const GlowingEffect = memo(({ blur = 0, inactiveZone = 0.7, proximity = 0, spread = 20, variant = "default", glow = false, className, movementDuration = 2, borderWidth = 1, disabled = true }: GlowingEffectProps) => {
  ...
  const handleMove = useCallback((e?: MouseEvent | { x: number; y: number }) => {
    ...
    animationFrameRef.current = requestAnimationFrame(() => {
      ...
      animate(currentAngle, newAngle, { duration: movementDuration, ease: [0.16, 1, 0.3, 1], onUpdate: (value) => { element.style.setProperty("--start", String(value)); } });
    });
  ...
  return (
    <>
      ...
      <div ref={containerRef} style={{ "--blur": `${blur}px`, "--spread": spread, "--start": "0", "--active": "0", "--glowingeffect-border-width": `${borderWidth}px`, "--repeating-conic-gradient-times": "5", "--gradient": variant === "white" ? `repeating-conic-gradient(from 236.84deg at 50% 50%, var(--black), var(--black) calc(25% / var(--repeating-conic-gradient-times)))` : `radial-gradient(circle, #4F46E5 10%, #4F46E500 20%), radial-gradient(circle at 40% 40%, #818CF8 5%, #818CF800 15%), radial-gradient(circle at 60% 60%, #10B981 10%, #10B98100 20%), radial-gradient(circle at 40% 60%, #6366F1 10%, #6366F100 20%), repeating-conic-gradient(from 236.84deg at 50% 50%, #4F46E5 0%, #818CF8 calc(25% / var(--repeating-conic-gradient-times)), #10B981 calc(50% / var(--repeating-conic-gradient-times)), #6366F1 calc(75% / var(--repeating-conic-gradient-times)), #4F46E5 calc(100% / var(--repeating-conic-gradient-times)))` } as CSSProperties} className={cn("pointer-events-none absolute inset-0 rounded-[inherit] opacity-100 transition-opacity", glow && "opacity-100", blur > 0 && "blur-[var(--blur)]", className, disabled && "!hidden")}>
